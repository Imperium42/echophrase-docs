...
DirectML is the default on Windows and needs no setup. If you already have
**CUDA Toolkit 13.0+** and **cuDNN 9.19+** installed, you can opt in to the
CUDA execution provider from **Settings -> GPU Acceleration**: Echophrase
downloads the CUDA runtime as a plugin (~650 MB) and activates it after a
restart. You do not need CUDA to get GPU acceleration.
