...
<Note>
Windows and macOS are in **Beta**. The Linux build is in **Alpha** - functional but rough around the edges (CPU-only), and not yet code-signed. All downloads resolve to the latest beta-channel build via CrabNebula Cloud's CDN.
</Note>

<Warning>
**macOS users:** the macOS Beta build is not yet notarized with Apple, so macOS will say Echophrase is "damaged" on first launch. After dragging to Applications, run this in Terminal to clear Apple's quarantine flag:
