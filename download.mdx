...
<CardGroup cols={3}>
  ...
  <Card title="macOS (Alpha)" icon="apple" href="https://cdn.crabnebula.app/download/imperium42/echophrase/latest/platform/macos-aarch64?channel=beta">
...
<Note>
Windows is in **Beta**. macOS and Linux builds are in **Alpha** - functional but rough around the edges, and not yet code-signed. All downloads resolve to the latest beta-channel build via CrabNebula Cloud's CDN.
</Note>

<Warning>
**macOS users:** the Alpha build is not yet notarized with Apple, so macOS will say Echophrase is "damaged" on first launch. After dragging to Applications, run this in Terminal to clear Apple's quarantine flag:
