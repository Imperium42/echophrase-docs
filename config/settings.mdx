...
CUDA on NVIDIA GPUs, DirectML on any Windows GPU, CoreML on Apple Silicon,
or NPU on Copilot+ PCs. CPU is the fallback when nothing else is available.

You can force a specific provider if you need to:

- **Auto (Recommended)**: Let Echophrase pick the fastest device at session start
- **CUDA**: Force NVIDIA CUDA (NVIDIA GPU with CUDA 12.8+ and cuDNN 9.19+ required)
